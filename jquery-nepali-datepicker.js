...
(function ($) {
  ...
  var defaults = {
    ...
    maxDate: null,              // e.g. '2081-12-30'
    disabledDates: [],          // array of BS strings or {year,month,day}
    disableBeforeToday: false,  // true => minDate = today
    disableAfterToday: false    // true => maxDate = today
  };
  ...
  // v5-compatible base pair:
  // REF_BS here is 2000-09-17 BS <-> REF_AD = 1944-01-01 AD (UTC)
  var REF_BS = { year: 2000, month: 9, day: 17 };
  var REF_AD = new Date(Date.UTC(1944, 0, 1));
  ...
  function adToBS(input, outAsString) {
    ...
    var REF_AD_LOCAL = { year: 1944, month: 1, day: 1 };
    var REF_BS_LOCAL = { year: 2000, month: 9, day: 17 };

    var t1 = Date.UTC(y, m - 1, d);
    var t0 = Date.UTC(REF_AD_LOCAL.year, REF_AD_LOCAL.month - 1, REF_AD_LOCAL.day);
    var diffDays = Math.round((t1 - t0) / DAY_MS);

    var by = REF_BS_LOCAL.year, bm = REF_BS_LOCAL.month, bd = REF_BS_LOCAL.day + diffDays;

    if (diffDays >= 0) {
      while (true) {
        var dm = GetDaysInMonth(by, bm);
        if (bd <= dm) break;
        bd -= dm; bm++; if (bm > 12) { bm = 1; by++; if (!isYearValid(by)) { by = Math.max.apply(Math,Object.keys(bsCalendarData).map(Number)); bd = Math.min(bd, GetDaysInMonth(by,bm)); break; } }
      }
    } else {
      while (bd < 1) {
        bm--; if (bm < 1) { bm = 12; by--; if (!isYearValid(by)) { by = Math.min.apply(Math,Object.keys(bsCalendarData).map(Number)); bd = 1; break; } }
        bd += GetDaysInMonth(by, bm);
      }
    }

    var out = clampBS({ year: by, month: bm, day: bd });
  ...
  function getTodayADParts(settings){
    ...
    if (settings && typeof settings.tzOffset === 'number') {
      ...
      return { year: z.getUTCFullYear(), month: z.getUTCMonth()+1, day: z.getUTCDate() +1 };
    }
    // Fallback: device local (no +1 day!)
  ...
  function firstWeekday(bs){
    ...
    // Use UTC to avoid DST oddities
    return new Date(Date.UTC(a.year, a.month-1, a.day)).getUTCDay();
  }
  ...
  // constraints helpers
  function parseBS(input){
    if (!input) return null;
    if (typeof input === 'string'){
      var m = /^(\d{4})-(\d{1,2})-(\d{1,2})$/.exec(input.trim());
      return m ? {year:+m[1], month:+m[2], day:+m[3]} : null;
    }
    if (typeof input === 'object' && input.year && input.month) {
      return {year:+input.year, month:+input.month, day:+(input.day||1)};
    }
    return null;
  }
  function keyBS(d){ return d.year+'-'+String(d.month).padStart(2,'0')+'-'+String(d.day).padStart(2,'0'); }
  function normalizeConstraints(settings){
    var today = getTodayBS(settings);
    var minBS = settings.disableBeforeToday ? today : parseBS(settings.minDate);
    var maxBS = settings.disableAfterToday  ? today : parseBS(settings.maxDate);

    var set = new Set();
    (settings.disabledDates||[]).forEach(function(x){
      var b = parseBS(x);
      if (b) set.add(keyBS(clampBS(b)));
    });

    return {minBS:minBS?clampBS(minBS):null, maxBS:maxBS?clampBS(maxBS):null, disabledSet:set};
  }
  function isOutOfRange(date, minBS, maxBS){
    if (minBS && cmp(date, minBS) < 0) return true;
    if (maxBS && cmp(date, maxBS) > 0) return true;
    return false;
  }

  // local helper for positioning: current open picker
  function getOpenPickerEl(){
    if (ACTIVE) return $(ACTIVE);
    var $v = $('.nepali-datepicker:visible').first();
  ...
  $.fn.nepaliDatepicker = function (options) {
    ...
    return this.each(function () {
      ...
      // constraints
      var constraints = normalizeConstraints(settings);
      $input.data('__ndp_constraints__', constraints);

      // initialize with TODAY
      var t0 = getTodayBS(settings);
      var state = {
        isOpen: false,
        selected: { year: t0.year, month: t0.month, day: t0.day },
        current:  { year: t0.year, month: t0.month, day: t0.day },
        view: 'month',
        $dp: null,
        $overlay: null,
        bound: false
        ...
        state.$dp.on('click.ndp', '[data-action]', function(e){
          ...
          switch(action){
            ...
            case 'select-day':
              ...
              var cand = { year: cur.year, month: cur.month, day: d };
              var C = $input.data('__ndp_constraints__') || {minBS:null, maxBS:null, disabledSet:new Set()};
              if (isOutOfRange(cand, C.minBS, C.maxBS) || C.disabledSet.has(keyBS(cand))) return; // constrained
              state.selected = cand;
              $input.val(fmt(settings, state.selected));
            ...
            case 'today':
              ...
              var CX = $input.data('__ndp_constraints__') || {minBS:null, maxBS:null, disabledSet:new Set()};
              if (isOutOfRange(t, CX.minBS, CX.maxBS) || CX.disabledSet.has(keyBS(t))) return;
      ...
      $input.data('nepaliDatepicker', {
        ...
        setDate: function(date){
          ...
          // respect constraints
          var C = $input.data('__ndp_constraints__') || {minBS:null, maxBS:null, disabledSet:new Set()};
          if (isOutOfRange(bs, C.minBS, C.maxBS) || C.disabledSet.has(keyBS(bs))) return;
        ...
        destroy: function(){
          ...
          $input.off('.ndp')
                ...
                .removeData('__ndp_constraints__')
  ...
  /*** ---------- Small public converters (optional) ---------- ***/
  ...
  // local positioning util (exposed for app code if needed)
  window.__ndp_positionBelow = function($input){
    var $dp = getOpenPickerEl();
    if (!$dp) return;
    var wasHidden = $dp.css('display') === 'none';
    if (wasHidden) $dp.css({ display: 'block', visibility: 'hidden' });
    ...
    var spaceBelow = (st + vh) - (off.top + ih);
    var spaceAbove = (off.top - st);
    var top;
